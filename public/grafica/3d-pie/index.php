...
Highcharts.chart('container', {
    ...
    series: [{
        ...
        data: [
        <?php 
        $sql ="SELECT Cantidad, PiezasHechas FROM TblSesiones WHERE IdSesion=".$Id;
        $result = mysqli_query($connection,$sql);
        while ($sesiones = mysqli_fetch_array($result)){
            $Restante = $sesiones["Cantidad"] - $sesiones["PiezasHechas"];
          ?>
            ['<?php echo $sesiones["PiezasHechas"];
            ?>', <?php echo $sesiones["PiezasHechas"] ?>],
            ['<?php echo $Restante;
            ?>', <?php echo $Restante ?>],
          <?php
