$(document).ready(function(){
	...
	$('.nav-btn-submenu1').on('click', function(e){
		e.preventDefault();
	...
	$('.btn-exit-system').on('click', function(e){
		...
		Swal.fire({
			title: 'CERRAR SESIÓN',
			text: "¿Está seguro de cerrar la su sesión y salir del sistema?",
			type: 'question',
			...
			if (result.value) {
				window.location=baseurl + 'Login/cerrar_sesion';
                //alert(baseurl+'Login/login');
			}
		});
	});
});
